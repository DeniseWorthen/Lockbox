...
!===============================================================================
  ...
  subroutine med_time_alarmInit( clock, alarm, option, &
       opt_n, opt_ymd, opt_tod, reftime, alarmname, advance_clock, alarmtype, rc)
    ...
    character(len=*) , optional , intent(in)    :: alarmtype     ! on intervals or at set times
    ...
    if (alarmtype == 'interval') then
       ...
       if (maintask) then
          write(logunit,*)
          write(logunit,'(a)') trim(subname) //' creating interval alarm '// trim(lalarmname)
       end if
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       call ESMF_ClockGet(clock, currTime=alarmTime, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       ...
       if (maintask) then
          write(logunit,*)
          write(logunit,'(a,i6)') trim(subname) //' creating set time alarm '// trim(lalarmname) &
               //' at hour ',opt_n
       end if

       !call ESMF_AlarmPrint(alarm,rc=rc)
       !call ESMF_AlarmPrint(alarm,options="enabled",rc=rc)
       !call ESMF_AlarmPrint(alarm,options="ringBegin",rc=rc)
       !call ESMF_AlarmPrint(alarm,options="ringDuration",rc=rc)
       !call ESMF_AlarmPrint(alarm,options="sticky",rc=rc)
    end if
