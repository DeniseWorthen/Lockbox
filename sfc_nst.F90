...
module sfc_nst
  ...
  use machine
  use med_kind_mod          , only : kind_phys=>SHR_KIND_R8
  use funcphys              , only : fpvs
  use module_nst_water_prop , only : get_dtzm_point

  use module_nst_parameters , only : hvap => con_hvap, cp => con_cp, hfus => con_hfus, &
       jcal => con_jcal, eps => con_eps, epsm1 => con_epsm1, rvrdm1 => con_fvirt,      &
       rd => con_rd, rhow = > con_rhw0, pi => con_pi, tgice => con_tice, sbc => con_sbc
  ...
  use module_nst_water_prop , only : solar_time_from_julian, density,rhocoef,          &
       compjd,grv,sw_ps_9b
...
contains
  ...
  ! t1=GFS_Data(cdata%blk_no)%Statein%tgrs(:,1), u1,v1,q1 similar
  ...
  ! tref,tskin,tsurf intent(out) from pre
  subroutine sfc_nst_run                                          &
       ( im,  ps, u1, v1, t1, q1, tref, cm, ch,          &
       lseaspray, fm, fm10,                                       &
       prsl1, prslki, prsik1, prslk1, xlon, sinlat, stress,       &
       sfcemis, dlwflx, sfcnsw, rain, timestep, solhr, xcosz,     &
       wind, flag_iter, flag_guess, nstf_name1, nstf_name4,       &
       nstf_name5, thsfc_loc,                                     &
    ...
    !          ( im, ps, u1, v1, t1, q1, tref, cm, ch,                      !
    !            lseaspray, fm, fm10,                                       !
    !            prsl1, prslki, xlon, sinlat, stress,  !
    !            sfcemis, dlwflx, sfcnsw, rain, timestep,solhr,xcosz,       !
    !            wind,  flag_iter, flag_guess, nstf_name1, nstf_name4,      !
    !            nstf_name5, thsfc_loc,                                     !
    !       input/outputs:                                                  !
    ...
    !  ---  inputs:
    integer,                             intent(in) :: im, nstf_name1, nstf_name4, nstf_name5
    real (kind=kind_phys), dimension(:), intent(in) :: ps, u1, v1, t1, q1, tref, cm, ch, fm,     &
         fm10, prsl1, prslki, prsik1, prslk1, xlon, xcosz, sinlat, stress, sfcemis, dlwflx,      &
         sfcnsw, rain, wind
    ...
    real (kind=kind_phys), dimension(:), intent(inout) :: tskin, tsurf, xt, xs, xu, xv, xz,      &
         zm, xtts, xzts, dt_cool, z_c, c_0, c_d, w_0, w_d, d_conv, ifd, qrain
    ...
    !
    logical :: flag(im)   !TODO: for so_omask=1
    real (kind=kind_phys), dimension(im) :: xt_old, xs_old, xu_old, xv_old, xz_old,zm_old,       &
         xtts_old, xzts_old, ifd_old, tref_old, tskin_old, dt_cool_old,z_c_old

    real(kind=kind_phys) :: ulwflx(im), nswsfc(im)
    real(kind=kind_phys) :: alpha,beta,rho_w,f_nsol,sss,sep, cosa,sina,taux,tauy,grav,dz,t0,     &
         ttop0,ttop
    ...
    !integer :: iw3jdn !TODO julian day
    !
    !  parameters for sea spray effect
    !
    real (kind=kind_phys) :: f10m, u10m, v10m, ws10, ru10, qss1, bb1, hflxs, evaps, ptem
    real (kind=kind_phys), parameter :: alps=0.75,bets=0.75,gams=0.15, ws10cr=30., conlf=7.2e-9, &
         consf=6.4e-8
    !
    !======================================================================================================
       ...
       if ( flag(i) ) then
          ...
          if ( (soltim > solar_time_6am .and. ifd(i) == zero) ) then
             ...
             if ( f_nsol > zero .and. xt(i) > zero ) then
                call convdepth(timestep,nswsfc(i),f_nsol,sss,sep,rho_w
             ...
             call dtm_1p(timestep,rich,taux,tauy,nswsfc(i),
             ...
             if ( xt(i) > zero ) then
                ...
                !! thickness of \a dz.
                if ( q_warm > zero ) then
                   call cal_ttop(timestep,q_warm,rho_w,dz,
                   &                        xt(i),xz(i),ttop0)
          ...
          if ( xt(i) > zero ) then
             call cal_w(xz(i),xt(i),xzts(i),xtts(i),w_0(i),w_d(i))
          else
             w_0(i) = zero
             w_d(i) = zero
          endif
