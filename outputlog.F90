...
contains
  ...
  subroutine outputlog_run(mclock, atStopTime, rc)
    ...
    ! both last and next-to-last files are written w/ prefix=currtime, which is the same!
    ! maybe send actual prefix, instead of logical to add a prefix that is found in shr_log
    ! olog(n)%logprefix = ....
    ! if atstop and not lstop, prefix=....
    ! if atstop and lstop, prefix=....
