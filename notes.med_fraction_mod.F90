...
!================================================================================================
  ...
  subroutine med_fraction_init(gcomp, rc)
       ...
       if (med_map_RH_is_created(is_local%wrap%RH(compice,compatm,:),mapfcopy, rc=rc)) then
          ...
          if (.not. med_map_RH_is_created(is_local%wrap%RH(compice,compatm,:),maptype, rc=rc)) then
             call med_map_routehandles_init( compice, compatm, &
                  ...
                  mapindex=maptype, RouteHandle=is_local%wrap%RH(compice,compatm,:), rc=rc)
             if (ChkErr(rc,__LINE__,u_FILE_u)) return
          end if
       end if
    ...
    if ( is_local%wrap%comp_present(compocn) .and. &
       ...
       ! XXX
       ! sa_ofrac = (1-ifrac)*(1-lfrac); time evolving; not being mapped here---it is already on the atm
       ! grid
       ! XXX
  ...
  subroutine med_fraction_set(gcomp, rc)
    ...
    if (is_local%wrap%comp_present(compice)) then
       ...
       ! ofrac = si_imask - (si_ifrac*si_imask)
       ! ofrac = si_imask*(1-si_ifrac)
       ! ofrac = si_omask*(1-si_ifrac); si_omask = (1-lfrac)?
       ! fv3 uses ofrac = (1-si_ifrac)(1-ofrac)?
       ! XXX
       if (associated(ofrac)) then
       ...
       if (is_local%wrap%comp_present(compatm)) then
          ...
          ! XXX
          ! ofrac = si_imask - (si_ifrac*si_imask)
          ! ofrac = si_imask*(1-si_ifrac); si_imask=si_omask
          ! ofrac = si_omask*(1-si_ifrac); si_omask = (1-lfrac)?
          ! fv3 uses ofrac = (1-si_ifrac)(1-ofrac)?
          ! XXX
    ...
    ! XXX
    ! ifrac from fbfrac(compice) => mapconsd => mapped ifrac in fbfrac(compatm)
    ! ofrac from fbfrac(compice) => mapconsd => mapped ofrac in fbfrac(compatm)
    ! XXX
    !---------------------------------------
    ! Diagnostic output
    !---------------------------------------
