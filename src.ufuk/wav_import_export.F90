...
contains
  ...
  subroutine import_fields( gcomp, time0, timen, rc )
    ...
    if (INFLAGS1(3)) then
      ...
      WX0(:,:) = def_value
      WXN(:,:) = def_value
      print*, "state_fldchk(importState, trim(uwnd)) = ", state_fldchk(importState, trim(uwnd))
      if (state_fldchk(importState, trim(uwnd))) then
        ...
        call ESMF_LogWrite(trim(subname)//' standalone part before ', ESMF_LOGMSG_INFO)
        if (standalone) then
           call ESMF_LogWrite(trim(subname)//' standalone run. reading atm u wind data from wind.ww3', ESMF_LOGMSG_INFO)
           print*, "wxdata min, max = ", minval(wxdata, dim=1), maxval(wxdata, dim=1)
           print*, "WX0 min, max = ", minval(WX0), maxval(WX0)
           print*, "WXN min, max = ", minval(WXN), maxval(WXN)
           call FillGlobalInput(wxdata, WX0)
           call FillGlobalInput(wxdata, WXN)
        end if
        call ESMF_LogWrite(trim(subname)//' standalone part after ', ESMF_LOGMSG_INFO)
      end if
      ...
      if (state_fldchk(importState, trim(vwnd))) then
        ...
        if (standalone) then
           ...
           print*, "wydata min, max = ", minval(wydata, dim=1), maxval(wydata, dim=1)
           print*, "WY0 min, max = ", minval(WY0), maxval(WY0)
           print*, "WYN min, max = ", minval(WYN), maxval(WYN)
  ...
  subroutine fillglobal_with_import(global_data, globalfield)
    ...
    call ESMF_LogWrite(' here before', ESMF_LOGMSG_INFO)
    do isea = 1,nsea
      ix = mapsf(isea,1)
      iy = mapsf(isea,2)
      globalfield(ix,iy) = global_data(isea)
    end do
    call ESMF_LogWrite(' here after', ESMF_LOGMSG_INFO)
