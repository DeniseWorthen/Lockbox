...
! authors: William H. Lipscomb, LANL
      ...
      do i = ilo, ihi
         ...
         ijb(3) = iblk
         if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,3g15.7)','XX bf therm1 ',istep,aice (i,j, iblk), vice(i,j, iblk),vsno (i,j, iblk)
      ...
      !-----------------------------------------------------------------
         ...
         if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,3g15.7)','XX af therm1',istep,aice (i,j, iblk), vice(i,j, iblk),vsno (i,j, iblk)
      enddo ! i
      enddo ! j
...
!          Elizabeth C. Hunke, LANL
	 ...
	 if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,1g15.7)','XX b4 therm2 ',istep,aice (i,j, iblk)
      ...
      if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,1g15.7)','XX af therm2 ',istep,aice (i,j, iblk)
      enddo                     ! i
      enddo                     ! j
      ...
      do iblk = 1, nblocks
         ...
         do j = jlo, jhi
            do i = ilo, ihi
               ...
               ijb(2) = this_block%j_glob(j)
               ijb(3) = iblk
               if (ijb(1).eq.40 .and. ijb(2).eq.7 .and. ijb(3).eq.1)print '(a,2i5,10g15.7)','AAA4a b4 transport ',ijb(1:2),vicen(i,j,  :,iblk),aicen(i,j,  :,iblk)
            end do
         end do
      end do
      ...
      do iblk = 1, nblocks
         ...
         do j = jlo, jhi
            do i = ilo, ihi
               ...
               ijb(2) = this_block%j_glob(j)
               ijb(3) = iblk
               if (ijb(1).eq.40 .and. ijb(2).eq.7 .and. ijb(3).eq.1)print '(a,2i5,10g15.7)','AAA4b af transport ',ijb(1:2),vicen(i,j,  :,iblk),aicen(i,j,  :,iblk)
            end do
         end do
      end do
...
!      if (atmp > c0) then
         ...
         ijb(2) = this_block%j_glob(j)
         ijb(3) = iblk
         if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,3g15.7)','XX bf stepridge ',istep,aice (i,j, iblk), vice(i,j, iblk),vsno (i,j, iblk)
            ...
            if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,3g15.7)','XX af stepridge ',istep,aice (i,j, iblk), vice(i,j, iblk),vsno (i,j, iblk)
         endif ! tmask
...
!          Nicole Jeffery, LANL
      ...
      do i = ilo, ihi
         ...
         ijb(2) = this_block%j_glob(j)
         ijb(3) = iblk
         if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,1g15.7)','XX b4 step snow ',istep,aice (i,j, iblk)
         call icepack_step_snow (dt = dt,                &
                     ...
                     fsloss = fsloss  (i,j,iblk),        &
                     fsnow  = fsnow   (i,j,iblk))
         if(ijb(1) .eq. 40 .and. ijb(2) .eq. 7)print '(a,i8,1g15.7)','XX af step snow ',istep,aice (i,j, iblk)
      enddo
      enddo
