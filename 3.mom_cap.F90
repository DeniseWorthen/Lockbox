...
subroutine ModelAdvance(gcomp, rc)
  !debug
  use ESMF, only : ESMF_TimeIntervalSet, ESMF_AlarmWasPrevRinging, ESMF_AlarmWillRingNext
  use netcdf
  ...
  ! debug
  logical :: chk4output = .false.
  integer :: nlen, ncid, dimid
  character(len=256) :: fname = ''
  logical :: exists
  type(ESMF_TimeInterval) :: elapsedTime, outputInterval
  integer(kind=8) :: elapsed_secs
  logical :: wasRinging, willRing
  integer :: ringcnt
  !type(ESMF_Clock)                       :: oclock
  ...
  call ESMF_ClockGetAlarm(oclock, alarmname='history_alarm', alarm=history_alarm, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  !call ESMF_AlarmGet(history_alarm, ringingOnPrevTimeStep=wasRinging, timeStepRingingCount=ringcnt, rc=rc)
  !if (ChkErr(rc,__LINE__,u_FILE_u)) return
  !if(is_root_pe()) print *,'XXX checking for prev ringing '//trim(export_timestr),'  ',ringcnt,wasRinging
  willRing=ESMF_AlarmWillRingNext(history_alarm, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  if(is_root_pe()) print *,'XXX checking for next ringing '//trim(export_timestr),'  ',willring
  ...
  ! if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ! if (ESMF_AlarmWasPrevRinging(history_alarm, rc=rc)) then
  !    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  !    if(is_root_pe()) print *,'XXX prev ringing '//trim(export_timestr)
  ...
  if (chk4output) then
    if(is_root_pe()) then
      ...
      if (exists) then
        ...
        rc = nf90_close(ncid)
        if (nlen > 0) then
          print *,trim(export_timestr)//' XX '//trim(fname)//' complete'
          !chk4ouput = .false.
        else
          print *,trim(export_timestr)//' XX '//trim(fname)//' still 0'
          !chk4output = .true.
        end if
      end if
    end if
  ...
  if (ESMF_AlarmIsRinging(history_alarm, rc=rc)) then
    ...
    call ESMF_ClockGetNextTime(clock, MyTime, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_TimeIntervalSet(outputInterval, h=3, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

    call ESMF_TimeGet (MyTime-outputInterval, yy=year, mm=month, dd=day, h=hour, m=minute, s=seconds, rc=rc )
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    write(fname,'(a,i4.4,3(a,i2.2),a)')'ocn_',year,'_',month,'_',day,'_',hour,'.nc'
    !write(timestamp,'(".",i4.4,"-",i2.2,"-",i2.2,"-",i5.5)'),year,month,day,hour*3600+minute*60+seconds
    if(is_root_pe()) print *,'XXX0 '//trim(fname),'  ',trim(export_timestr)
    !end if
  ...
  ! !call ESMF_ClockGetAlarm(clock, alarmname='history_alarm', alarm=history_alarm, rc=rc)
  ! !if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ! call ESMF_AlarmGet(history_alarm, ringingOnPrevTimeStep=wasRinging, &
  !      timeStepRingingCount=ringcnt, rc=rc)
  ! if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ! if(is_root_pe()) print *,'XXX checking for prev ringing '//trim(export_timestr),'  ',ringcnt
  ! if (wasRinging) then
  !    if(is_root_pe()) then
  !       ! check if file is written
  !       inquire(file=trim(fname), exist=exists)
...
subroutine ModelSetRunClock(gcomp, rc)
  use ESMF, only : ESMF_ClockCreate, ESMF_TimeIntervalSet
  ...
  ! debug
  !type(ESMF_Clock)         :: oclock
  type(ESMF_TimeInterval)  :: otimestep
  !--------------------------------
  ...
  ! copy model clock and set 6 hour interval
  oclock = ESMF_ClockCreate(mclock, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  call ESMF_TimeIntervalSet(otimestep, h=6, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  call ESMF_ClockSet(oclock, currTime=dcurrtime, timeStep=otimestep, stopTime=mstoptime, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  if (first_time) then
    ...
    ! create an alarm for closing of history output
    !call ESMF_TimeIntervalGet(dtimestep, m=dt_cpl, rc=rc)
    !if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call AlarmInit(oclock,        &
         alarm   = history_alarm, &
         option  = 'nhours',      &
         opt_n   = 6,             &
         opt_ymd = -999,          &
    ...
    call ESMF_AlarmSet(history_alarm, clock=oclock, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  call ESMF_ClockAdvance(oclock,rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  call ESMF_ClockSet(oclock, currTime=dcurrtime, timeStep=otimestep, stopTime=mstoptime, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
