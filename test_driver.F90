...
program test_driver
  ...
  !testrings = [6,12]
  !testticks = [-1,1]
  !call run_case(name=trim(testname), freq=testfreq, start_hour=teststart, ring_hours=testrings, &
  !     ring_ticks=testticks,     &
  !     use_filesize = .true.,    &
  ...
  call assert_equal(completions, , testname, assertrc, assertmsg)
  ...
  call assert_equal(completions, 0, testname, assertrc, assertmsg)
  call addresult(freqntests, assertrc, trim(assertmsg), '')
  ! ------------------
...
!#endif
  ...
  call addresult(freqntests, assertrc, trim(assertmsg), '')
  ! TODO
  ! add test that doesn't start at 6 for freq=6; say start=9, freq=6 to test offset
  ! same for 24
  ! ------------------
  ! Test results
  ! ------------------
...
contains
  ...
  subroutine run_case(test, freq, start_hour, ring_hours, ring_ticks, completions, &
    ...
    runhours = 3*freq + 1
    !! maybe create a setup_case w/ the cfn,staten initializzation, alarminit ~ outputlog_init
    call setup_case(runhours, freq, l_nfiles, l_timereduce, modelClock, cf_n, state_n, rc)
