...
contains
  ...
  use mom_outputlog_methods , only : XX
  use netcdf
    ...
    do n = 1,n_freq
      ...
      if (chour(1:2) == output_fh(1:2)) then
        ...
        if (ESMF_AlarmIsRinging(olog(n)%alarm, rc=rc)) then
          ...
          ! fname = construct_filename(nextTime,olog(n)%filename_fhoffset,olog(n)%avgtype,olog(n)%filename_root,rc=rc)
          ...
          existflag = (nlen(1) /= nf90_fill_int)
          if (existflag) then
