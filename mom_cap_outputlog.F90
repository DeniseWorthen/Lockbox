  ...
  do n = 1,n_freq
    ...
    if (chour(1:2) == output_fh(1:2)) then
      ...
      if (ESMF_AlarmIsRinging(olog(n)%alarm, rc=rc)) then
        ...
        importexport = get_importexport(currTime, nextTime, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        inquire(file="MOM6_OUTPUT/ocn_2021_03_22_12_00.nc", exist=existflag)
        if (existflag) then
          if (is_root_pe())print '(a)','XXX  MOM6_OUTPUT/ocn_2021_03_22_12_00.nc exist '//trim(importexport)
        endif

        inquire(file="MOM6_OUTPUT/ocn_2021_03_22_18_00.nc", exist=existflag)
        if (existflag) then
          if (is_root_pe())print '(a)','XXX  MOM6_OUTPUT/ocn_2021_03_22_18_00.nc exist '//trim(importexport)
        endif

        inquire(file="MOM6_OUTPUT/ocn_2021_03_23_00_00.nc", exist=existflag)
        if (existflag) then
          if (is_root_pe())print '(a)','XXX  MOM6_OUTPUT/ocn_2021_03_23_00_00.nc exist '//trim(importexport)
        endif
        ...
        if (existflag) then
          if (is_root_pe())then
            nlen(1) = get_unlimited_len(trim(tmpname))
            inquire(file=trim(tmpname), size=fsize(1))
            print '(a,i6,i16,L)','XXX  '//trim(tmpname)//' exist '//trim(importexport)//' ',&
                 nlen(1),fsize(1),olog(n)%chkfile_nextAdvance
          endif
        endif
        ...
        tmpname = "MOM6_OUTPUT/ocn_2021_03_22_10_00.nc"
        importexport = get_importexport(currTime, nextTime, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        inquire(file=trim(tmpname), exist=existflag)
        if (existflag) then
        ...
        tmpname = "MOM6_OUTPUT/ocn_2021_03_22_11_00.nc"
        importexport = get_importexport(currTime, nextTime, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        inquire(file=trim(tmpname), exist=existflag)
        if (existflag) then
        ...
        tmpname = "MOM6_OUTPUT/ocn_2021_03_22_12_00.nc"
        importexport = get_importexport(currTime, nextTime, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        inquire(file=trim(tmpname), exist=existflag)
        if (existflag) then
      ...
      if (olog(n)%chkfile_nextAdvance) then
        ...
        !importexport = get_importexport(currTime, nextTime, rc=rc)
        !inquire(file=fname, size=fsize(1))
        !if (is_root_pe())print '(a,2i16)','XXX call filecomplete '//trim(importexport)//'  '//trim(fname),olog(n)%createsize,fsize(1)
        filecomplete = file_is_complete(fname, olog(n)%use_filesize, olog(n)%createsize, rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        !if (is_root_pe())print '(a,L)','XXX post filecomplete '//trim(importexport)//'  '//trim(fname),filecomplete
      ...
      if (lstop) then
        ...
        if (existflag) then
          if (is_root_pe())then
            nlen(1) = get_unlimited_len(trim(tmpname))
            inquire(file=trim(tmpname), size=fsize(1))
            print '(a,i6,i16,L)','XXX  '//trim(tmpname)//' exist lstop '//trim(importexport)//' ',&
                 nlen(1),fsize(1),olog(n)%chkfile_nextAdvance
          endif
        endif
        tmpname = "MOM6_OUTPUT/ocn_2021_03_22_08_00.nc"
...
logical function file_is_complete(fname, chk4size, createsize, rc) result(filecomplete)
  ...
  !if (is_root_pe()) then
  !  print *,'XXX in filecomplete '//trim(fname),fsize(1),createsize
  !end if
  if (chk4size) then
