...
type(outputlog_model_times_type) :: times
...
subroutine outputlog_init(gcomp, mclock, ocean_grid, rc)
  ...
  cf(:)%outputdir = trim(dirs%output_directory)
  ...
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  call ESMF_TimeIntervalSet(times%tincrement, m=1, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
...
subroutine outputlog_run(mclock, atStopTime, rc)
  ...
  call ESMF_ClockGet(mclock, startTime=times%startTime, currTime=times%currTime, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  call ESMF_ClockGetNextTime(mclock, times%nextTime, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  !importexport = get_importexport(times%currTime, times%nextTime, rc=rc)
  !if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  do n = 1,n_freq
    if (cf(n)%requested) then
      call ESMF_ClockGetAlarm(mclock, alarmname=trim(cf(n)%alarm_name), alarm=cf_n%alarm, rc=rc)
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
      ...
      !! don't need to send the clock!
      call outputlog_freqn(times, cf(n), state(n), mpicomm, &
           is_root_pe(), root_pe(), lastrestart, debug_onroot, atStopTime=lstop, rc=rc)
...
!! @param[out]    rc                     return code
subroutine outputlog_freqn(times, cf_n, state_n, mpicomm, isroot, rootpe, &
     , lastrestart, debug_onroot, atStopTime, rc)

  !type(ESMF_Clock),             intent(in)    :: mclock
  type(outputlog_model_times_type), intent(in) :: times
  ...
  ! when the alarm rings, set file check on next advance and construct the filename
    if (state_n%ringing) then
      ...
      timestr = get_timestr(times%nextTime-cf_n%filename_fhoffset, rc=rc)
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
      state_n%filename = trim(cf_n%outputdir)//trim(cf_n%fnameprefix)//trim(timestr)//'.nc' &
    ...
    !call check_file_completion(state_n, lastrestart, mpicomm, isroot, rootpe, times%startTime, &
    !     logtime=times%currTime-cf_n%logname_fhoffset, complog='mom6.'//chour, rc=rc)
    !rc = merge(ESMF_SUCCESS, ESMF_FAILURE, rc == 0)
    !if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    !
    if (state_n%filecomplete) call log_file_complete(....)

    if (state_n%filecomplete) then
      get_file_state(comm, isroot, rootpe, state_n%filename, state_n%completesize, rc)
      state_n%chkfile_nextAdvance = .false.
      state_n%time_lastrestart = lastrestart

      if (isroot) then
        call log_restart_fh(logtime=currTime-cf_n%logname_fhoffset, startTime, complog, prefixtime=.true., &
             lastrestart=state_n%time_lastrestart, lastoutput=state_n%filename, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
      endif
      !call log_file_complete(isroot, rootpe, state_n, outputdir)
    endif
    ...
    if (lstop) then
      ...
      call get_file_state(mpicomm, isroot, rootpe, state_n%filename, fsize=fsize, nlen=nlen,rc=rc)
      rc = merge(ESMF_SUCCESS, ESMF_FAILURE, rc == 0)
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
      ...
      state_n%filecomplete = (fsize == state_n%completesize .and. nlen == 1)
      state_n%chkfile_nextAdvance = .false.
      state_n%time_lastrestart = lastrestart
      if (isroot) then
