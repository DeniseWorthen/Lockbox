...
contains
  ...
  subroutine wav_pio_init(gcomp, mpi_comm, stdout, numprocs, rc)
    ...
    ! if (my_task == 0) write(stdout,*) trim(subname),' calling pio_set_rearr_opts'
    ! ierr = pio_set_rearr_opts(wav_pio_subsystem, pio_rearr_comm_type, pio_rearr_comm_fcd, &
  ...
  subroutine wav_pio_initdecomp_3d(nz, iodesc)
    ...
    do k = 1,nz
      do jsea = 1,nseal_cpl
        ...
        iy = mapsf(isea,2)                             ! global iy
        n = n+1
        dof3d(n) = (int((iy-1),kind=PIO_OFFSET_KIND)*lnx + ix) &
    ...
    !call pio_writedof('gfs.dof.bin', (/nx,ny,nz/), dof3d, mpi_comm_wave)
    !call pio_write_nc_dof(wav_pio_subsystem, 'gfs.dof.nc', PIO_64BIT_DATA, iodesc, ierr, fortran_order=.true.)
    deallocate(dof3d)
