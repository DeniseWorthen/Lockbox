...
!> Index of the namelist slot in nml_fh holding target_freq, or 0 if none
!! does. Pure lookup only -- does not report ierr/errmsg itself; callers
!! that rely on a match existing (settype, setprefix) must check for 0
!! themselves and fail the same way they already do for any other invalid
!! input, since a match is only guaranteed by the calling convention
!! (requested(n) true implies setrequest already found a matching nml_fh
!! entry), not enforced by this function.
...
function setprefix(validfreqs, requested, nml_fh, nml_fnameprefix, errmsg, ierr) result(fileprefixes)
  ...
  if (n_active == 1) then
    n = findloc(requested, .true., dim=1)   ! exactly one true value -- find it directly, no loop needed
