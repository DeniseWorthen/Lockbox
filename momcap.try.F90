...
subroutine ModelAdvance(gcomp, rc)
  ...
  call ESMF_ClockGetAlarm(hclock, alarmname=trim('history_alarm'), alarm=history_alarm, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
...
subroutine ModelSetRunClock(gcomp, rc)
  ...
  if (first_time) then
    ...
    call alarmInit(hclock, history_alarm, option='nhours', opt_n=6, &
         reftime=StartTime, alarmname=trim('history_alarm'), advance_clock=.true., rc=rc)
