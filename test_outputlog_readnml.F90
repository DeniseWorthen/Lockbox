...
program test_outputlog_readnml
  ...
  type(testsummary)  :: nmltests(maxtests)

  !logical :: expected_result
  logical :: is_passing
  integer :: i,nt,n,ierr
  !integer :: npass = 0
  !integer :: nfail = 0
  ...
  !expected = (ierr == 0 .and. .not. any(requested))
  is_passing = (ierr == 0 .and. .not. any(requested))

  call assert_logical(is_passing, .true., testname, assertrc, assertmsg)

  call addresult(nmltests, assertrc, assertmsg)
  !if (is_passing) then
  !   npass = npass + 1
