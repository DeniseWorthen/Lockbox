program testvars
  ...
  !integer, parameter :: noswll = 3 + 1 ! 0:noswll
  !integer, parameter :: ef_len = 25    ! e3df(3,1) - e3df(2,1) + 1
  ...
  integer    ,pointer           :: dimid(:)
   ...
   gridoutvar(4,1:17) = [ &
    varatts( "PH   ", "PHS       ", "Partitioned wave heights                        ", "m    ", "x y s ", .false.) , &
   ...
   if(s_axis)ierr = nf90_def_dim(ncid,'noswll',noswll+1,stid)
   if(b_axis)ierr = nf90_def_dim(ncid,'nb',   3,        btid)
   if(m_axis)ierr = nf90_def_dim(ncid,'nm',   (p2msf(3)-p2msf(2))+1,mtid)
   if(p_axis)ierr = nf90_def_dim(ncid,'np',   USSPF(2),ptid)
   if(k_axis)ierr = nf90_def_dim(ncid,'freq', nk, ktid)

   ! define the variables
   do j = 1,nogrp
    do i = 1,maxvars
     if(gridoutvar(j,i)%validout) then
      if(scan(trim(gridoutvar(j,i)%dims),'s') > 0) then
        dimids = (/timid, stid, ytid, xtid/)
      else if(scan(trim(gridoutvar(j,i)%dimids),'b') > 0) then
       dimids = (/timid, btid, ytid, xtid/)
      else if(scan(trim(gridoutvar(j,i)%dimids),'m') > 0) then
       dimids = (/timid, mtid, ytid, xtid/)
      else if(scan(trim(gridoutvar(j,i)%dimids),'p') > 0) then
       dimids = (/timid, ptid, ytid, xtid/)
      else if(scan(trim(gridoutvar(j,i)%dimids),'k') > 0) then
       dimids = (/timid, ktid, ytid, xtid/)
      else
       dimids = (/timid, ytid, xtid/)
      end if
      ierr = nf90_def_var(ncid, trim(gridoutvar(j,i)%varname), nf90_float, dimids, varid)
      ierr = nf90_put_att(ncid, varid, '_FillValue', undef)
      ierr = nf90_put_att(ncid, varid, 'units', trim(gridoutvar(j,i)%unit_name))
      ierr = nf90_put_att(ncid, varid, 'long_name', trim(gridoutvar(j,i)%long_name))
      ierr = nf90_enddef(ncid)

   do j = 1,nogrp
    do i = 1,maxvars
     if(gridoutvar(j,i)%validout) then
      if(trim(gridoutvar(j,i)%varname) .eq. 'WX')call write_var(trim(fname), trim(gridoutvar(j,i)%varname), trim(..dims), DW)
      if(trim(gridoutvar(j,i)%varname) .eq. 'CX')call write_var(trim(fname), trim(gridoutvar(j,i)%varname), trim(..dims), CX)
      if(trim(gridoutvar(j,i)%varname) .eq. 'CY')call write_var(trim(fname), trim(gridoutvar(j,i)%varname), trim(..dims), CY)
