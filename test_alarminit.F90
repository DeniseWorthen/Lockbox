...
program test_alarminit
  ...
  use ESMF,  only : ESMF_CALKIND_GREGORIAN, ESMF_Calendar, ESMF_Alarm
  ...
  call run_case(freq=testfreq,  start_hour=teststart, ring_hour, errmsg, ierr)

  is_passing = (ierr == 0 .and. mod(ring_hour,6) == 0)
  call assert_equal(is_passing, .true., testname, assertrc, assertmsg)
  call addresult(alarmtests, assertrc, trim(assertmsg))

  ! send optional max steps which would override 200 and/or override dt
  ! get back ierr = 1; report is passing if ierr /= 0
...
contains
  ...
  subroutine run_case(freq, start_hour, err_count)
    ...
    integer, parameter :: max_steps = 200   ! 100h at test dt=30min
    ...
    call ESMF_TimeSet(startTime, yy=base_yy, mm=base_mm, dd=base_dd, h=start_hour, calendar=cal, rc=ierr)
    ...
    if (verbose) then
       if (freq >= 6) then
          print '(A,I0,A)', " Model start hour is not on a 6h boundary by ", toffset,"h used as RefTime"
       else
          print '(A)', " Ouput freq < 6: no toffset correction applies, regardless of start_hour alignment"
       end if
    end if
    ...
    if (.not. rang) then
       ...
       if (verbose) call assert_true(rang, trim(label)//": alarm rang within "//itoa(max_steps)//" steps", err_count)
    end if

    call ESMF_ClockGet(clock, currTime=ringTime, rc=ierr)
    call esmf_err(ierr, "ESMF_ClockGet(currTime at ringTime)")

    call ESMF_TimeGet(ringTime, dd=ring_day, h=ring_hour, rc=ierr)
    call esmf_err(ierr, "ESMF_TimeGet(ringTime)")

    if (verbose) then
       print '(3(A,I0))', " Alarm rang at step = ", step,", ring day = ", ring_day, " hour = ", ring_hour
    end if

    ! Validate ring hour against frequency
    if (freq >= 6) then
       ...
       time_equal = (startTime + ringInterval == ringTime)
       nt = nt+1; msg = trim(label)//' Ring hour must be start+freq exactly'
       call assert_equal(time_equal, .true., trim(msg), rc, msg_out)
       call accumulate_results(nt, rc, trim(msg_out))
       !call assert_time_equal(expectedTime, ringTime, &
       !     trim(label)//": Ring hour must be start+freq exactly", err_count)
    end if
    ...
    call assert_true(ring_hour == expected_hour, &
         trim(label)//": Manual ring hour calculation "//itoa(expected_hour)//", got "//itoa(ring_hour), err_count)
  ...
  subroutine assert_true(condition, msg, err_count)
    ...
    assertmsg =  ' ASSERTION '//trim(passfail)//": "//trim(msg)

    if (verbose) then
       print '(A)',trim(assertmsg)
    else
       if (passfail == 'FAILED') print '(A)',trim(assertmsg)
    endif
