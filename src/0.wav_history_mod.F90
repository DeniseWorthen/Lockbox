  ...
  subroutine write_history ( timen )
    ...
    do n = 1,size(outvars)
      ...
      if (trim(outvars(n)%dims) == 's') then
        ...
        if(vname .eq.     'PDIR') call write_var3d(iodesc3ds, vname, pdir     (1:nseal_cpl,0:noswll), fldir='false' )
        if(vname .eq.      'PSI') call write_var3d(iodesc3ds, vname, psi      (1:nseal_cpl,0:noswll), fldir='false' )
        if(vname .eq.      'PWS') call write_var3d(iodesc3ds, vname, pws      (1:nseal_cpl,0:noswll) )
        if(vname .eq.      'PDP') call write_var3d(iodesc3ds, vname, pthp0    (1:nseal_cpl,0:noswll), fldir='false' )
        ...
        if (vname .eq.     'THM') call write_var2d(vname, thm      (1:nseal_cpl), fldir='false' )
        if (vname .eq.     'THS') call write_var2d(vname, ths      (1:nseal_cpl), fldir='false' )
        if (vname .eq.    'THP0') call write_var2d(vname, thp0     (1:nseal_cpl), fldir='false' )
