...
contains
  ...
  subroutine waveinit_ufs( gcomp, stdout, ntrace, mpi_comm, mds, rc)
    ...
    fnmpre = './'
    !inquire(unit=stdout,name=cvalue)
    !print *,'XXX ',iaproc,stdout,trim(cvalue)
    if (root_task) write(stdout,'(a)') trim(subname)//' call read_shel_config'
    call read_shel_config(mpi_comm, mds, time0_overwrite=time0, timen_overwrite=timen)
    !flush(stdout)
    ! nds is set == mds in w3init

    ! needed only so that logfile is written correctly
    !call mpi_barrier(mpi_comm, ierr)

    call ESMF_LogWrite(trim(subname)//' call w3init', ESMF_LOGMSG_INFO)
    call w3init ( 1, .false., 'ww3', mds, ntrace, odat, flgrd, flgr2, flgd, flg2, &
         npts, x, y, pnames, iprt, prtfrm, mpi_comm )
    !print *,'XXX ',iaproc,root_task,nds(1),stdout,mds(1),dtmax,dtcfl,dtcfli,dtmin
    !flush(stdout)
    ! call mpi_barrier ( mpi_comm, ierr )
    ! do i=1,size(nds)
    !   inquire(unit=nds(i),opened=is_open)
    !   if (is_open) then
    !     flush(nds(i))
    !     print *,iaproc,' XXX flushing unit = ',i
    !   end if
    ! end do

    !inquire(unit=stdout,name=cvalue,opened=is_open)
    !print *,'YYY ',iaproc,stdout,nds(1),root_task,is_open,trim(cvalue),dtmax,dtcfl,dtcfli,dtmin
    !flush(stdout)
    !write(logmsg,*)trim(subname)//': mod_def timesteps file  ',dtmax,dtcfl,dtcfli,dtmin
    !if (root_task) write(stdout,'(a)') trim(logmsg)

    !if (root_task) write(stdout,'(a,4f10.1)') trim(subname)//': mod_def timesteps file0  ',dtmax,dtcfl,dtcfli,dtmin
    !if (root_task) write(stdout,*) trim(subname)//': mod_def timesteps file1  ',dtmax,dtcfl,dtcfli,dtmin
    ...
    if (isPresent .and. isSet) then
      ...
      !write(cvalue,'(4f10.1)')dtmax,dtcfl,dtcfli,dtmin
      !if (root_task) write(stdout,'(a)') trim(subname)//': mod_def timesteps reset  '//trim(cvalue)

      !if (root_task) write(stdout,*) trim(subname)//': mod_def timesteps reset  ',dtmax,dtcfl,dtcfli,dtmin
      !write(logmsg,*)trim(subname)//': mod_def timesteps reset  ',dtmax,dtcfl,dtcfli,dtmin
      !if (root_task) write(stdout,'(a)') trim(logmsg)
      !if (root_task) write(stdout,'(a,4f10.1)') trim(subname)//': mod_def timesteps reset  ',dtmax,dtcfl,dtcfli,dtmin
    end if
