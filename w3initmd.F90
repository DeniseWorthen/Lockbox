...
CONTAINS
  ...
  SUBROUTINE W3INIT ( IMOD, IsMulti, FEXT, MDS, MTRACE, ODAT, FLGRD,  FLGR2, FLGD, &
    ...
    use w3odatmd,        only : runtype, restart_from_binary, use_restartnc, user_restfname, use_historync
...
9033 FORMAT ( ' +------+------+------+------+--------------+',    &
  ...
  SUBROUTINE W3MPIO ( IMOD )
    ...
    IF ((FLOUT(4) .OR. FLOUT(8)) .and. (.not. LPDLIB) .and. do_setup) THEN
      IF (OARST) THEN
      ...
      END IF
      !
    END IF ! IF ((FLOUT(4) .OR. FLOUT(8)) .and. (.not. LPDLIB) .and. do_setup) THEN
#endif
    !
