...
CONTAINS
  ...
  SUBROUTINE W3WAVE ( IMOD, ODAT, TEND, STAMP, NO_OUT &
    ...
    IF ( DTTST .LT. 0. ) THEN
      ...
      CALL EXTCDE ( 1 )
    END IF
    if (iaproc .eq. 3)print '(a,2i10,2i10,f10.2,L)','DEBUGX 1a',TIME,TEND,DTTST,FLZERO
    !
    ...
    IF ( FLCUR ) THEN
      ...
      END IF
    END IF
    if (iaproc .eq. 3)print '(a,2i10)','DEBUGX curr',TOFRST
    !
    ! 1.d Wind interval
    !
    IF ( FLWIND ) THEN
...
#endif
    if (iaproc .eq. 3)print '(a,2i10)','DEBUGX wind',TOFRST
    !
...
#endif
    if (iaproc .eq. 3)print '(a,2i10)','DEBUGX icec',TOFRST
    !
...
#endif
    !
    if (iaproc .eq. 3)print '(a,2i10,2i10)','DEBUGX start ',TOFRST,TONEXT(:,1)
    ...
    FLFRST = .TRUE.
    DO
      if (iaproc .eq. 3)print '(a,2i10)','DEBUGX start section 2 ',TONEXT(:,1)
#ifdef W3_TIMINGS
          ...
          ELSE ! not using Cheng method
            ...
            IF (FLIC1.AND.FLIC2.AND.FLIC3.AND.FLIC4) THEN
              IF (ICEP1(IX,IY)>0.0) THEN ! non-zero ice
                CALL W3IC3WNCG_V1(WN(:,ISEA),WN_I(:), CG(:,ISEA),ICEP1(IX,IY),ICEP2(IX,IY), &
                     ICEP3(IX,IY),ICEP4(IX,IY),DEPTH)
              END IF ! non-zero ice
            ELSE
              IF ( IAPROC .EQ. NAPERR ) WRITE(NDSE,*)'ICE PARAMETERS NOT AVAILABLE FOR CG CALC'
              CALL EXTCDE(2)
            END IF
          ENDIF ! IF USE_CHENG...
#endif
      ...
      ELSE
        DTTST  = 0.
      ENDIF
      if(iaproc .eq. 3)print '(a,f10.2,2i10,2i10,2i10)','DEBUGX DTTST ',DTTST,TEND,TOFRST,TONEXT(:,1)
      !
      IF ( DTTST.GE.0. ) THEN
        TCALC = TEND
      ELSE
        TCALC = TOFRST
      END IF
      if(iaproc .eq. 3)print '(a,f10.2,2i10,2i10)','DEBUGX DTCALC ',DTTST,TEND,TOFRST
      !
      DTTST  = DSEC21 ( TIME , TCALC )
      NT     = 1 + INT ( DTTST / DTMAX - 0.001 )
      if(iaproc .eq. 3)print '(a,i4,3f10.2,2i10,2i10)','DEBUGX NT ',NT,DTTST,DTMAX,DTTST / DTMAX - 0.001,TIME,TCALC
      ...
      !
      DTRES  = 0.
      if(iaproc .eq. 3)print '(a,3i4,L)','DEBUGX start loop ',it0,nt,itime,flzero

      !
      DO IT = IT0, NT
        if(iaproc .eq. 3)print '(a,i5,2i10,2i10)','DEBUGX IT loop ',it,time,TONEXT(:,1)
#ifdef W3_TIMINGS
        ...
        ELSE IF ( FLFRST ) THEN
          RHOAIR = DAIR
        END IF
        if(iaproc .eq. 3)print '(a)','DEBUGX here  0'
        !
...
#endif
        if(iaproc .eq. 3)print '(a)','DEBUGX here 1'
        !
        ...
        IF ( FLIC1 .AND. DTI10.NE.0. ) THEN
          ...
          !
        END IF
        if(iaproc .eq. 3)print '(a)','DEBUGX here 2'
        !
...
#endif
        if(iaproc .eq. 3)print '(a)','DEBUGX here 3'
        !
        ...
        IF ( FLDRY .OR. IAPROC.GT.NAPROC ) THEN
          ...
          GOTO 380
        END IF
        if(iaproc .eq. 3)print '(a)','DEBUGX here 4'
        !
        ...
        END IF
#endif
        if(iaproc .eq. 3)print '(a)','DEBUGX here 6'
        !
        ! 3.6 Perform Propagation = = = = = = = = = = = = = = = = = = = = = = =
        ! 3.6.1 Preparations
        !
...
#endif
        if(iaproc .eq. 3)print '(a)','DEBUGX here 7'
          !
...
380     CONTINUE
        ...
        !
        !
        if (iaproc .eq. 3)print '(a,i5,2i10,2i10)','DEBUGX ENDDO ',it,TOFRST,TONEXT(:,1)
      END DO ! DO IT = IT0, NT
      ...
      !
400   CONTINUE
      if (iaproc .eq. 3)print '(a,i5,2i10)','DEBUGX 400 ',it,TOFRST
      !
      ! 4.  Perform output to file if requested ---------------------------- /
      ...
      IF ( DTTST.LE.0. .AND. DTTST1.NE.0. .AND. FLAG_O ) THEN
        ...
        WRITE (NDST,9041)
#endif
        if (iaproc .eq. 3)print '(a,i5,2i10,2i10)','DEBUGX 4a ',it,TOFRST,TONEXT(:,1)
        !
        ...
        !
        DO J=1, NOTYPE
        if (iaproc .eq. 3)print '(a,2i5,2i10,2i10)','DEBUGX 4b ',it,j,TOFRST,TONEXT(:,j)
          IF ( FLOUT(J) ) THEN
            !
            !
            ...
            !
          END IF
          if (iaproc .eq. 3)print '(a,i5,2i10)','DEBUGX NOTYPE',j,TOFRST
          !
