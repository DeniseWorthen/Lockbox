...
contains
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
    ...
    ! print *,'DEBUG startTime ',yy,mm,dd,start_tod

    ! call ESMF_ClockGet( clock, currTime=esmfTime, rc=rc)
    ! call ESMF_TimeGet( esmfTime, yy=yy, mm=mm, dd=dd, s=start_tod, rc=rc )
    ! print *,'DEBUG currTime ',yy,mm,dd,start_tod

    ! call ESMF_ClockGet( clock, stopTime=esmfTime, rc=rc)
    ! call ESMF_TimeGet( esmfTime, yy=yy, mm=mm, dd=dd, s=start_tod, rc=rc )
    ! print *,'DEBUG stopTime ',yy,mm,dd,start_tod

    ! ! get start/reference time
    ! call ESMF_ClockGet( clock, refTime=esmfTime, rc=rc)
    ! if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ! call ESMF_TimeGet( esmfTime, yy=yy, mm=mm, dd=dd, s=start_tod, rc=rc )
    ! print *,'DEBUG refTime ',yy,mm,dd,start_tod
    ...
    if (multigrid) then
      ...
      if ( trim(ifname) == 'ww3_multi.nml' ) then
        call wminitnml ( idsi, idso, idss, idst, idse, trim(ifname), mpi_comm, preamb=preamb )
      else
        call wminit ( idsi, idso, idss, idst, idse, trim(ifname), mpi_comm, preamb=preamb )
      endif
  ...
  subroutine waveinit_ufs( gcomp, ntrace, mpi_comm, mds, rc)
    ...
    use wav_shel_inp , only : flgrd, flgd, flgr2, flg2
    ! debug
    use w3odatmd     , only : notype, tofrst,tonext
    ...
    call ESMF_LogWrite(trim(subname)//' call read_shel_config', ESMF_LOGMSG_INFO)
    call read_shel_config(mpi_comm, mds, time0_overwrite=time0, timen_overwrite=timen)
    print *,'DEBUG shel config time0 ',time0
    print *,'DEBUG shel config timen ',timen
    ...
    if (iaproc .eq. 3) then
      do j = 1,notype
        print '(a,i5,2i10,2i10)','DEBUGX INIT ',j,TOFRST,TONEXT(:,j)
      end do
    end if
    if (dbug_flag > 5) call ESMF_LogWrite(trim(subname)//' done', ESMF_LOGMSG_INFO)
