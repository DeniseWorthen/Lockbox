...
contains
  ...
  !> @author mvertens@ucar.edu, Denise.Worthen@noaa.gov
  !> @date 09-12-2022
  subroutine write_meshdecomp(EMeshIn, mesh_name, nlen, rc)
    ...
    ! debug
    use ESMF          , only : ESMF_INDEX_GLOBAL, ESMF_INDEX_DELOCAL, ESMF_Index_Flag
    use ESMF, only : ESMF_FieldGet, ESMF_FieldWrite

    !use yownodepool, only : iplg,ipgl,npa,ng
    !use w3parall          , only : init_get_isea, pdlib_nseal
    !use wav_import_export , only : nseal_cpl
    use w3odatmd      , only : iaproc
    !use w3gdatmd      , only : nseal,trigp, ntri, nx, ny, mapsf
    use w3gdatmd , only : trigp
    ...
    type(ESMF_Field)               :: connfield
    character(len=6), dimension(5) :: lfieldlist
    integer                        :: i,ndims,nelements
    real(r8), pointer              :: fldptr1d(:)
    integer(i4), allocatable       :: dof(:)
    integer(i4), pointer           :: dofptr(:)
    integer(i4), pointer           :: connectionptr(:,:)
    ...
    !integer :: isea, jsea,n, ix, iy, i1,i2,i3
    !integer(i4), allocatable :: ltri(:,:)
    integer :: jsea
    !-------------------------------------------------------
    ...
    !print '(a,11i8)','XXXX ',iaproc,ntri,size(trigp,1),size(trigp,2),nseal,nseal_cpl,pdlib_nseal,npa,ng,size(ipgl),size(iplg)

    !allocate(ltri(size(trigp,1),size(trigp,2)))
    !ltri = trigp
    !do jsea = 1,nseal_cpl
    !  call init_get_isea(isea, jsea)
    !  connectionptr(:,jsea) = trigp(:,isea)
    !end do
    ...
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    print *,'XXXX nelements = ',nelements

    lfieldlist = (/'nconn ', 'dof   ', 'coordx', 'coordy', 'decomp'/)

    ! triangle connection field
    connfield = ESMF_FieldCreate(EMeshIn, ESMF_TYPEKIND_I4, name=trim(lfieldlist(1)), meshloc=ESMF_MESHLOC_ELEMENT, &
         ungriddedLbound=(/1/), ungriddedUbound=(/3/), gridToFieldMap=(/2/), rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldBundleAdd(FBTemp, (/connfield/), rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    ! index field
    doffield = ESMF_FieldCreate(EMeshIn, ESMF_TYPEKIND_I4, name=trim(lfieldlist(2)), &
         meshloc=ESMF_MESHLOC_ELEMENT, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldBundleAdd(FBTemp, (/doffield/), rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    ! coords and decomp field
    do i = 3,size(lfieldlist)
    ...
    ! if unstr....
    !call ESMF_FieldBundleGet(FBtemp, fieldName='nconn', field=lfield, rc=rc)
    !if (chkerr(rc,__LINE__,u_FILE_u)) return
    !call ESMF_FieldGet(lfield, farrayPtr=connectionptr, rc=rc)
    !if (chkerr(rc,__LINE__,u_FILE_u)) return
    !print *,'XXXX ',lbound(connectionptr,1),ubound(connectionptr,1),lbound(connectionptr,2),ubound(connectionptr,2)
    !connectionptr = 0
    !do jsea = 1,nseal_cpl
    !  call init_get_isea(isea, jsea)
    !  connectionptr(:,jsea) = trigp(:,isea)
    ...
    !  iy = mapsf(isea,2)                 ! global iy
    !  n = n+1
    !  connectionptr(:,n) = trigp(:,(iy-1)*nx + ix)         ! local index : global index
    !end do

    ! do jsea = 1,nseal_cpl
    !   print '(a,7i8)','YYYY ',iaproc,jsea,iplg(jsea),dof(jsea),trigp(:,iplg(jsea))
    !   i1=trigp(1,iplg(jsea)); i2=trigp(2,iplg(jsea)); i3=trigp(3,iplg(jsea))
    !   if (i1.eq.2.and.i2.eq.99.and.i3.eq.98)print *,'XYXY ',iaproc,jsea,dof(jsea)
    ! end do

    ! !globals
    ! !if (iaproc .eq. 1) then
    !   do n = 1,size(trigp,2)
    !     write(1010+iaproc,'(4i8)')n,trigp(:,n)
    !   end do
    ! !end if

    ! do n = 1,size(ipgl)
    !   write(2020+iaproc,'(2i8)')n,ipgl(n)
    ! end do

    ! do n = 1,size(iplg)-ng
    !   write(3030+iaproc,'(2i8)')n,iplg(n)
    ! end do

    call ESMF_FieldBundleGet(FBtemp, fieldName='nconn', field=connfield, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldGet(connfield, farrayPtr=connectionptr, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    !only for unstr, so iy==1
    !trigp is global
    ! do jsea = 1,nseal_cpl
    !   call init_get_isea(isea, jsea)
    !   ix = mapsf(isea,1)
    !   iy = mapsf(isea,2)
    !   print '(a,6i8)','AAAA ',iaproc,jsea,ix,trigp(:,ix)
    !   connectionptr(1:3,jsea) = trigp(1:3,ix)
    ! end do
    do jsea = 1,nlen
      connectionptr(:,jsea) = trigp(:,dof(jsea))
    end do
    !ltri = 0
    !do j = 1,nseal_cpl
    !  connectionptr(:,) = ltri(:,:)
    ...
    !call ESMF_FieldWrite( connfield, "nconn.nc", overwrite=.true., rc=rc )
    !if (chkerr(rc,__LINE__,u_FILE_u)) return
    !do jsea = 1,nseal_cpl
    !  write(4040+iaproc,'(5i8)')jsea,dof(jsea),ltri(:,jsea)
    !end do

    ! n = 0
    ! do jsea = 1,nseal_cpl
    !   print '(a,7i8)','ZZZZ ',iaproc,jsea,dof(jsea),iplg(jsea),trigp(:,dof(jsea))
    !   connectionptr(:,jsea) = trigp(:,dof(jsea))
    !   call init_get_isea(isea, jsea)
    !   ix = mapsf(isea,1)                 ! global ix
    !   iy = mapsf(isea,2)                 ! global iy
    !   ! n = n+1
    !   ! print '(a,7i8)','AAAA ',iaproc,jsea,ix,iy,n,(iy-1)*nx + ix,dof(jsea)
    !   ! connectionptr(:,n) = trigp(:,(iy-1)*nx + ix)         ! local index : global index
    !   print '(a,7i8)','AAAA ',iaproc,jsea,dof(jsea),connectionptr(:,jsea),(iy-1)*nx + ix
    ! end do
    ! call ESMF_FieldBundleGet(FBtemp, fieldName='nconn', field=lfield, rc=rc)
    ! if (chkerr(rc,__LINE__,u_FILE_u)) return
    ! call ESMF_FieldGet(lfield, farrayPtr=connectionptr, rc=rc)
