module vartypedefs
  ...
  type :: vardefs
     ...
     character(len=40)   :: long_name         !< A variable's long name
     character(len=10)   :: unit_name         !< A variable's unit
...
contains
  ...
  subroutine ocnvars_typedefine
    ...
    ii = ii + 1; ocnvars(ii)%input_var_name  = 'SSU' ; ocnvars(ii)%var_grid = 'Cu'
    ii = ii + 1; ocnvars(ii)%input_var_name  = 'SSV' ; ocnvars(ii)%var_grid = 'Cv'
