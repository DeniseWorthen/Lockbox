...
CONTAINS
  ...
  SUBROUTINE W3IOPP ( NPT, XPT, YPT, PNAMES, IMOD, MPI_COMM_IOPP )
    ...
    integer :: IERR_MPI
#endif
    integer, allocatable :: notfound(:), foundonland(:)
    !/
    !/ ------------------------------------------------------------------- /
    !/
    allocate(notfound(npt))
    allocate(foundonland(npt))
    notfound = 0
    foundonland = 0
#ifdef W3_S
    ...
    IF (.NOT. pnt_wght_exists) THEN
      ...
      if (iaproc .eq. naperr) then
        ...
        ! log the foundonland list
        do ipt = 1,npt
          if (foundonland(ipt) .eq. 1 ) then
            IF ( FLAGLL ) THEN
