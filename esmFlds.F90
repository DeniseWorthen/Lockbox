...
!================================================================================
  ...
  subroutine med_fldList_Realize(lpet,state, fldList, flds_scalar_name, flds_scalar_num, &
       grid, mesh, tag, rc)
  ...
  contains  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    subroutine SetScalarField(field, flds_scalar_name, flds_scalar_num, rc)
      ...
      use ESMF, only : ESMF_KIND_R8, ESMF_FieldGet
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=u_FILE_u)) return
      if(lpet==0)write(*,*)'MEDBB0 '//trim(tag(44:)),flds_scalar_num,size(farrayptr,1),size(farrayptr,2),farrayptr
      if(lpet/=0)write(*,*)'MEDBBX '//trim(tag(44:)),flds_scalar_num,size(farrayptr,1),size(farrayptr,2),farrayptr
      farrayptr = 0.0
