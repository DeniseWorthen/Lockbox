...
subroutine ModelAdvance(gcomp, rc)
  ...
  if (do_advance) then
    ...
    if (is_root_pe()) then
       ...
       print *,trim(export_timestr)//' XX0 '//trim(fname)
       if (exists) then
  ...
  if (mod(elapsed_secs,6*3600) == 0) then
    call ESMF_TimeIntervalSet(outputInterval, h=3, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_TimeGet (MyTime-outputInterval, yy=year, mm=month, dd=day, h=hour, rc=rc )
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    write(fname,'(A,i4.4,"_",i2.2,"_",i2.2,"_",i2.2,A)'),'ocn_',year,month,day,hour,'.nc'

    if (is_root_pe()) then
      print '(2a,7i8)','XXX ',trim(export_timestr)//'  '//trim(fname),elapsed_secs,year,month,day,hour
    end if
  end if
