...
program test_driver
  ...
  ! - this test tracks only files completed during normal polling; see the
  !   test_driver_lstop test for stoptime tests
  ! ===========================================================================
...
contains
  ...
  subroutine run_case(test, freq, start_hour, runhours, ring_hours, ring_ticks, &
    ...
    logical :: pending, firstcomplete, phantom_file
    ...
    do while (.not. ESMF_ClockIsStopTime(modelClock, rc=rc))
       ...
       if (state_n%ringing) then
          phantom_file = .false.
          ! if (l_timereduce == 'none') then
          !    if (modeltime%startTime > modeltime%nextTime) phantom_file = .true.
          !    timestr = get_timestr(modeltime%nextTime - cf_n%filename_fhoffset,rc=rc)
          !    print *,'XXX '//timestr//'  '//importexport
          ! else
          !    if (modeltime%startTime > modeltime%nextTime - cf_n%filename_fhoffset) phantom_file = .true.
          ! endif
          !if (.not. phantom_file) then
          timestr = get_timestr(modeltime%nextTime - cf_n%filename_fhoffset, rc=rc)
          if (modeltime%nextTime - cf_n%filename_fhoffset <= modeltime%startTime) phantom_file = .true.
          print *,'XXX '//timestr//'  '//importexport//' ',phantom_file
