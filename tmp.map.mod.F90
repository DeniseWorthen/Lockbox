...
!================================================================================
  ...
  subroutine med_map_RouteHandles_initfrom_esmflds(gcomp, flds_scalar_name, llogunit, rc)
    ...
    type(ESMF_FiedBundle)     :: FBdststatus
    integer                   :: n1,n2
    integer                   :: nf
    integer                   :: fieldCount
    ...
    ! create dststatusfield, doesn't need to be named
    ! call FieldCreate(....)
    ! create FB to hold the returned dststatusfields
                ...
                do while(associated(fldptr))
                   ...
                   if (mapindex /= mapunset) then
                      ...
                      if (mapindex /= mapfcopy) then
                         if (dststatus_print) then
                            ! dststatusfldnames(ncomps,nmappers)
                            ! dststatusfldnames(compid(n2),mapindex) = ...
                            ! create named R4 field and transfer I4 field to it
                            ! name src.dst.mapname
                            ! add field to FBdsttatus
                         end if
                      end if
    ...
    if (dststatus_print) then
       call write_dststatus_fields(gcomp, FBdststatus, dststatusfldnames, rc=rc)
       ...
       do n = 2,ncomps
          do n = 1,nmappers
             fldlist = anyfield with compname(n) as (1:3)

             call med_io_write(FBdststatus, flds=fldlist,lnx,lny....)
          end do
  ...
  !================================================================================
  subroutine med_map_routehandles_initfrom_field(n1, n2, fldsrc, flddst, mapindex, routehandles, mapfile, &
       dststatusfield, rc)
