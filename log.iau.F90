...
contains
  ...
  subroutine outputlog_init(gcomp, mclock, rc)
    ...
    if (is_root_pe())print *,'XXX ROOT on ',iam
    ...
    do n = 1,n_freq
      ...
      if (chour(1:2) == output_fh(1:2)) then
        ...
        if (olog(n)%chkfile_nextAdvance) then
          ...
          if (existflag) then
             !if root, then inquire, nlen(1)
             ! after return, call call ESMF_VMBroadCast(vm, nlen, 1, 0, rc=rc)
    ...
    if (is_root_pe()) then
       ...
       if (existflag) then
          ...
          if (is_root_pe()) then
             write(msgString,'(A)')trim(subname)//trim(fname)//' exists '//trim(importexport)
             if (nlen > 0) then
                print '(A)','XX0 '//trim(msgString)//' complete '
             else
                print '(A)','XX0 '//trim(msgString)//' still  0 '
             end if
          end if
       end if
