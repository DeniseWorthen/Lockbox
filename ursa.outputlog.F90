...
contains
  ...
  type(ESMF_Time),save         :: lastrestart
...
contains
        ...
        if (olog(n)%chkfile_nextAdvance) then
          ...
          if (existflag) then
            ...
            if (nlen > 0) then
              ...
              if (ChkErr(rc,__LINE__,u_FILE_u)) return
              if (is_root_pe()) then
                print *,'XXX0 b4 outputlog'
                 !call ESMF_TimePrint(currTime-60*freq(n)*timeoffset, options="string", rc=rc)
                 call ESMF_TimePrint(olog(n)%time_lastrestart, options="string", rc=rc)
                 if (ChkErr(rc,__LINE__,u_FILE_u)) return

                call log_restart_fh(currTime-60*freq(n)*timeoffset, startTime, 'mom6.'//chour, prefixtime=.true., &
                     lastwritten=olog(n)%filename, rc=rc)
                !call log_restart_fh(currTime-60*freq(n)*timeoffset, startTime, 'mom6.'//chour, prefixtime=.true., &
  ...
  subroutine outputlog_restart(mclock, num_rest_files, rc)
    ...
    if (all(allDone) .eqv. .true.) then
      ...
      print *,'XXX0 in lastrestart in restartlog'
      call ESMF_TimePrint(lastrestart, options="string", rc=rc)
