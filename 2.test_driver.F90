...
program test_driver
  ...
  ! ------------------
  nt = nt + 1
  expected = 1  ! stop at hour=18 completes 09 file
  write(testname,'(A,I2.2,A)')'test ',nt,' one tracked averaging window completes at stop'

  call run_case(trim(testname),                  &
       freq=6, start_hour=6, runhours=12,        &
       test_lstop = .true., use_filesize=.true., &
       completions=completions)
...
contains
  !> TODO
  subroutine run_case(test, freq, start_hour, runhours, timereduce, use_filesize, test_lstop, nfiles, &
       completions)
    ...
    logical,          intent(in), optional :: test_lstop
    ...
    l_test_lstop = .false.
    if (present(test_lstop)) l_test_lstop = test_lstop
    l_nfiles = 1
