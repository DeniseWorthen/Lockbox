...
!AR: todo call this only for global grid
  ...
  SUBROUTINE IS_IN_UNGRID(ipt, IMOD, XTIN, YTIN, ITOUT, IS, JS, RW)
    ...
    DO WHILE (nbFound.EQ.0.AND.ITRI.LT.GRIDS(IMOD)%NTRI)
      ...
      xavg=(x1+x2+x3)/3.0
      IF (ABS(XTIN-xavg).GT.180.0d0) THEN
      ...
      IF ((s1*sg1.GE.0.d0).AND.(s2*sg2.GE.0.0d0).AND.(s3*sg3.GE.0.0d0)) THEN
        itout=ITRI
        nbFound=nbFound+1
        IS(1)=I1
        IS(2)=I2
        ...
        RW(1)=s1/sg1
        RW(2)=s2/sg2
        RW(3)=1.d0-RW(1)-RW(2)  !s3/sg3
        RW(4)=0.d0
        !write(1000+iaproc,'(2(a,i6),3g14.7))')' point ',ipt,' not found at jsea ',jsea,s1*sg1,s2*sg2,s3*sg3
        write(2000+iaproc,'(2i8,2f10.2,3i8,3g14.7,3g14.7)')ipt,itri,xtin,ytin,is(1:3),rw(1:3),s1*sg1,s2*sg2,s3*sg3
      END IF
    ENDDO
