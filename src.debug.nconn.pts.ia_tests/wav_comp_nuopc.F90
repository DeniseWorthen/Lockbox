...
contains
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
    ...
    call ESMF_GridCompGet(gcomp, vm=vm, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_VMGet(vm, mpiCommunicator=mpi_comm, localPet=iam, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    ! npts, x, y, pnames are returned from w3init; the number of points, position and names
    ! nopts  is the "out points"....the ones found and mapped


    !print *,'BBB0 ',ntri,size(ielg)
    !do i = 1,size(ielg)
    !  write(7000+iaproc,'(6i8)'),i,ielg(i),trigp(1,ielg(i)),trigp(2,ielg(i)),trigp(3,ielg(i))
    !end do

    ! processor location of a node
    print *,'CCC0 ',size(ipgl_to_proc)
    !do i = 1,size(ipgl_to_proc)
    !  write(800+iaproc,'(2i8)')i,ipgl_to_proc(i)
    !end do
    ...
    i2 = trigp(2,ii)
    i3 = trigp(3,ii)
    print *,'AAA0 ii=47 ',i1,i2,i3
    print *,'AAA0 i1 ',ipgl_to_proc(i1)
    print *,'AAA0 i2 ',ipgl_to_proc(i2)
    print *,'AAA0 i3 ',ipgl_to_proc(i3)
    ...
    !   i2 = trigp(2,ii)
    !   i3 = trigp(3,ii)
    !   if (ipt .eq. i1 .or. ipt .eq. i2 .or. ipt .eq. i3) then
    !     print *,'DDD0 ',ipt,ii,i1,i2,i3
    !   end if
    ! end do
    ...
    do i = 1,size(ielg)
      ...
      i2 = trigp(2,ii)
      i3 = trigp(3,ii)
      !check that the nodes are on this DE
      !do i = 1,size(ipgl_to_proc)
      !  if
      ...
      do ipt = 1,npts
        if (.not. ptfound(ipt)) then
          !write(1000+iaproc,'(3(a,i6))')' working on jsea = ',jsea,' point ',ipt,' current count = ',lcnt
          ! point lat,lon
          xtin = real(x(ipt),8)
          ytin = real(y(ipt),8)
          !ensure xtin is defined with same coordinates as element
          if (abs(xtin-xavg) .gt. 180.d0) then
            xtinmod = xtin-sign(360.d0, (xtin-xavg))
          else
            xtinmod = xtin
          end if
          ...
          if ((s1*sg1 .ge. 0.d0) .and. (s2*sg2 .ge. 0.d0) .and. (s3*sg3 .ge. 0.d0)) then
            !itout=itri
            !nbfound=nbfound+1
            ptfound(ipt) = .true.
            lcnt = lcnt + 1
            ...
            rw(1,lcnt)=s1/sg1
            rw(2,lcnt)=s2/sg2
            rw(3,lcnt)=1.d0-rw(1,lcnt)-rw(2,lcnt)  !s3/sg3
            rw(4,lcnt)=0.d0
            write(1000+iaproc,'(2i8,2f10.2,i8,3i8,6g14.7)')ipt,ii,xtin,ytin,lcnt,is(1:3,lcnt),rw(1:3,lcnt),s1*sg1,s2*sg2,s3*sg3
            !write(1000+iaproc,*)ipt,gind,xtin,ytin,is,rw
          else
            !write(1000+iaproc,'(2(a,i6),3g14.7))')' point ',ipt,' not found at jsea ',jsea,s1*sg1,s2*sg2,s3*sg3
          end if
        end if
      end do
      !write(1000+iaproc,'(2(a,i6))')' done looking for jsea = ',jsea,' current count = ',lcnt
    end do
    do ipt = 1,npts
      if (.not. ptfound(ipt)) then
        write(900+iaproc,'(a,i6,2g14.7)')' not found: point ',ipt,x(ipt),y(ipt)
      end if
    end do
    ...
    !         rw(4)=0.
    !         write(1000+iaproc,'(3i8,2f10.2,3i8,3g14.7)')foundcount,ipt,gind,xtin,ytin,is(1:3),rw(1:3)
    !         !write(1000+iaproc,*)ipt,gind,xtin,ytin,is,rw
    !       end if
    !     end if
    !     !count = count + 1
    ...
    ! call mpi_bcast(npts,1,mpi_integer,iaproc-1,mpi_comm,ierr)
    ! call mpi_barrier(mpi_comm,ierr)

    ! call mpi_bcast(ptloc,2*npts,mpi_real,iaproc-1,mpi_comm,ierr)
    ! call mpi_bcast(ptifac,4*npts,mpi_real,iaproc-1,mpi_comm,ierr)
    ! call mpi_bcast(iptint(:,:,1:npts),2*4*nopts,mpi_integer,iaproc-1,mpi_comm,ierr)

    !   !send point names individually
    ! do ipt=1, nopts
    !   call mpi_bcast(ptnme(ipt),4iaproc-1,mpi_character,iaproc-1,mpi_comm,ierr)
    ! enddo
    !call mpi_barrier ( mpi_comm, ierr )
