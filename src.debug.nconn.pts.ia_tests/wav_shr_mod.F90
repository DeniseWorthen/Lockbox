...
contains
  ...
  subroutine write_meshdecomp(EMeshIn, mesh_name, nseal_cpl, rc)
    ...
    use ESMF, only : ESMF_MESHLOC_NODE
    ...
    !integer(i4), pointer           :: itri(:,:), itritmp(:,:)
    ...
    ! debug
    integer :: ii, icnt, node1, node2, node3
    integer :: foundnode(10)
    integer(i4), allocatable :: ltri(:,:)
    type(ESMF_DistGrid)            :: dgtmp
    !-------------------------------------------------------
    ...
    ! jsea is just index in 1:nseal_cpl
    ! gindex_sea(jsea) is global index of a *element* on the emsf mesh, or the index of a node for ww3
    ! these nodes are held on this PE in array ielg, where the ww3 nodes connected to form this element
    ! can be retrieved as trigp(1:3,ielg(i))
    ...
    !     node2 = trigp(2,ii)
    !     node3 = trigp(3,ii)
    !     if (isea .eq. node1 .or. isea .eq. node2 .or. isea .eq. node3) then
    !       icnt = icnt + 1
    !       foundnode(icnt) = ii
    !     end if
    !   end do
    !   write(8000+iaproc,'(3i6,10i6)')jsea,isea,icnt, foundnode(:)
    ! end do
    !128 2 3 0 128 0 196 179 49
    ...
    !     print *,'XYXY ',jsea,isea,itri(:,jsea)
    !   end do
    !   call ESMF_FieldBundleAdd(FBTemp, (/trifield/), rc=rc)
    !   if (chkerr(rc,__LINE__,u_FILE_u)) return
    ! end if
    ! print *,'YYY0',size(trigp,1),size(trigp,2)
    ! do i = 1,size(trigp,2)
    !   write(5000+iaproc,'(5i8)')ntri,i,trigp(:,i)
    ! end do
    ...
    do jsea = 1,nseal_cpl
      call init_get_isea(isea, jsea)
      !write(6000+iaproc,'(10i8)')ntri,jsea,isea,dof(jsea),trigp(:,jsea),trigp(:,isea)
      write(6000+iaproc,'(10i8)')ntri,jsea,isea,dof(jsea),ltri(:,jsea),ltri(:,isea)
      !if (dof(jsea) .eq. 550)print *,'DDD0 ',jsea,trigp(:,jsea)
    end do
    if (unstr_mesh) then
      ! create a temporary mesh with default distgrid where distgrid =1,ntri
      ! redist trifield onto this mesh
      ! write it out?

      !trifield = ESMF_FieldCreate(EMeshIn, ESMF_TYPEKIND_I4, name='nconn', meshloc=ESMF_MESHLOC_ELEMENT, &
      !     ungriddedLBound=(/1/), ungriddedUBound=(/size(trigp,1)/), gridToFieldMap=(/2/), rc=rc)
      !trifield = ESMF_FieldCreate(EMeshIn, ESMF_TYPEKIND_I4, name='nconn', meshloc=ESMF_MESHLOC_NODE, &
      !     ungriddedLBound=(/1/), ungriddedUBound=(/3/), gridToFieldMap=(/2/), rc=rc)
      !if (chkerr(rc,__LINE__,u_FILE_u)) return
      !call ESMF_FieldGet(trifield, farrayPtr=itri, rc=rc)
      !if (chkerr(rc,__LINE__,u_FILE_u)) return

      print *, 'DDD0 ',size(ltri,1),size(ltri,2)

      ! !call ESMF_FieldGet(trifield, farrayPtr=itrimp, rc=rc)
      ! !if (chkerr(rc,__LINE__,u_FILE_u)) return
      ! do jsea = 1,nseal_cpl
      !   call init_get_isea(isea, jsea)
      !   itri(:,jsea) = trigp(:,jsea)
      !   !itri(:,jsea) = 0
      !   !itri(:,jsea) = trigp(:,dof(jsea))
      !   !itri(:,jsea) = trigp(:,isea)
      ! end do
      ! call ESMF_FieldBundleAdd(FBTemp, (/trifield/), rc=rc)
      ! if (chkerr(rc,__LINE__,u_FILE_u)) return
