...
contains
  ...
  subroutine waveinit_ufs( gcomp, stdout, ntrace, mpi_comm, mds, rc)
    ...
    call read_timesteps_nml(...)
    sum = sum(timesteps)
    if (sum > 0.0) then
      reset_dts = .true.
      dt_reset(1) = timesteps%dtmax
      dt_reset(2) = timesteps%dtxy
      dt_reset(3) = timesteps%dtkth
      dt_reset(4) = timesteps%dtmin
    else
      reset_dts = .false.
    end if
    ...
    call w3init ( 1, .false., 'ww3', mds, ntrace, odat, flgrd, flgr2, flgd, flg2, &
         npts, x, y, pnames, iprt, prtfrm, mpi_comm , reset_dts)
